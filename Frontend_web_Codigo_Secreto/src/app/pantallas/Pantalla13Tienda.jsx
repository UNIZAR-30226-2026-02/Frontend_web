...
const themes = [
  { name: "Cyberpunk", price: "DESBLOQUEADO", emoji: "🌃", unlocked: true },
  { name: "Naturaleza", price: "500", emoji: "🌿", unlocked: false },
  { name: "Espacio", price: "750", emoji: "🚀", unlocked: false },
  { name: "Fantasía", price: "1000", emoji: "🔮", unlocked: false },
];
const packs = [
      ...
      <div className="max-w-4xl mx-auto pt-8 sm:pt-4">
        ...
        <ManilaFolder>
          <div className="p-4 sm:p-6 lg:p-8">
            ...
            {/* Board Themes */}
            <SubsectionLabel icon={<Palette className="w-4 h-4 text-[#5a4a30]" />} label="ESTÉTICA DE TABLERO" borderColor="#2a3a5a" />
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
              {themes.map((t) => (
                <DarkCard key={t.name} className="p-4 text-center">
                  <div className="text-3xl sm:text-4xl mb-2">{t.emoji}</div>
                  <p className="font-['Special_Elite',cursive] text-[#e8dcc8]" style={{ fontSize: 13 }}>{t.name}</p>
                  {t.unlocked ? (
                    <div className="mt-2 flex items-center justify-center gap-1 text-[#50a060]">
                      <Check className="w-4 h-4" />
                      <span className="font-['Courier_Prime',monospace]" style={{ fontSize: 10 }}>DESBLOQUEADO</span>
                    </div>
                  ) : (
                    <>
